...
- cd <folder>: Change Directory - Moves you into a specific folder.
...
- cp <source> <destination>: Copy - Makes a duplicate copy of a file.
- mv <source> <destination>: Move/Rename - Renames a file or moves it to another folder.
- rm <file>: Remove - Permanently deletes a file (Warning: No recycle bin!).

### 3. Viewing & Searching Data
- cat <file>: Displays the entire contents of a file on the screen.
- head <file>: Shows the first 10 lines of a file (useful for large genomic data).
- tail <file>: Shows the last 10 lines of a file.
- grep "<text>" <file>: Searches inside a file for a specific sequence motif or text line.
